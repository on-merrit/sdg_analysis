...
papers <- spark_read_csv(sc, "/user/tklebel/sdg/data/sdg_papers_collated.csv",
                         name = "papers")
# remove Virus for now
papers <- papers %>% 
  filter(fos_displayname != "Virus")
...
papers_per_affiliation_per_w_leiden %>% 
  mutate(in_leiden = !is.na(University)) %>% 
...
```{r sdg_who_total_n_by_country, fig.width=10, fig.height=8}
...
```{r sdg_who_mncs_by_country, fig.width=10, fig.height=8}
papers_per_country_fos_author_pos_country %>% 
  filter(author_position == "first_author") %>% 
  # mutate(mncs = mncs + .1) %>% 
  # filter(mncs < 10) %>% 
