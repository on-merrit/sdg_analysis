...
step1 <- papers %>% 
  ...
  filter(!is.na(journalid), year %in% 2015:2018) %>% 
...
p <- apc_affiliation_leiden %>% 
  ...
  labs(y = "Share of papers in category", colour = NULL,
       title = "Association between institutional prestige (2015-2018) and whether APCs are involved or not")
p
```
...
p2 <- apc_affiliation_leiden %>% 
  ...
  theme(legend.position = "top") +
  labs(y = "Share of papers in category", colour = NULL,
       title = "Association between institutional prestige (2015-2018) and whether APCs are ionvolved or not")
p2
```
